...
/**
 * Class is responsible for validation of input entered by user in the filling form window constructed by
 * {@link com.marcin.phone.views.FillingFormView} class.
 * It allows to check 3 text fields being filled by user: first name field, last name field and phone number field.
 * 
 * @author dream-tree
...
public class InputValidator {
	...
	public String checkInput(String firstName, String lastName, String number) {
		...
		try {
			...
			// all other numbers input errors (letters or integers exceeding the Integer type range).
			result = warning;
			e.getMessage();
		}
	...
	/**
	 * Gets the result of the initial user input check from {@link #checkInput(String, String, String)}
	 * and returns specialized info if input is not correct,
	 * or calls appropriate method for updating phone base: either for adding or modifying existing contact.
	 * Adding new contact requires additional validation operation - typed phone number must be unique in the phone base.
	 * If it is not, contact adding process is rejected.
	 * @param userChoice flag pointing out if user wants to add or to change existing contact
	 */
	...
	/**
	 * Performs additional validation operation if user wants to add a new contact to the phone base.
	 * Typed phone number must be unique in the phone base.
	 * If it is not, contact adding process is rejected.
	 * @param userChoice flag pointing out if user wants to add or to change existing contact
	 */
